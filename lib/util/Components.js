 ...
 * @author Yannick Croissant (https://github.com/yannickcr/eslint-plugin-react/blob/master/lib/util/Components.js)
 */
'use strict';
...
class Components {
  ...
  list() {
    ...
    for (const i in this._list) {
      ...
      if (component) {
        const newUsedProps = (this._list[i].usedPropTypes || []).filter(
...
function componentRule(rule, context) {
  ...
  const utils = {
    ...
    isReactCreateElement: function(node) {
      ...
      const calledDirectly = node && node.callee && node.callee.name === 'createElement';
    ...
    isReturningJSX: function(ASTnode, strict) {
      ...
      const returnsJSX = node[property] && node[property].type === 'JSXElement';
