    ...
    return {
      JSXAttribute: function(node) {
        if (
          ...
          (node.value.expression.properties[0].value.type !==
            "CallExpression" ||
            (node.value.expression.properties[0].value.type ===
              "CallExpression" &&
              node.value.expression.properties[0].value.callee.type !==
                "Identifier") ||
            (node.value.expression.properties[0].value.callee &&
              !(
                node.value.expression.properties[0].value.callee.name in
                SANITIZER_NAMES
              )))
        ) {
          context.report({
